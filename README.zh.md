...
- **操作审计日志**：每次扫描与删除都写入 `~/Library/Logs/MacBroom/macbroom.log`（可用 `MACBROOM_LOG_DIR` 覆盖目录），可事后追溯工具到底动了什么。
...
- **登录项体检**：揪出指向「已删除程序」的孤儿开机启动项（就是系统设置里报错的那些「后台项」）。用户级项可移入废纸篓，系统级项给出 `sudo` 命令由你自行执行（绝不替你提权）。
...
只对比 MacBroom 真正发力的维度。我们不列自己没有的功能——要恶意软件查杀、磁盘树状图、菜单栏组件，全功能原生应用更合适。

|  | **MacBroom** | CleanMyMac | Pearcleaner | Mole |
|---|:---:|:---:|:---:|:---:|
| **价格** | 免费 | $39.95/年 | 免费 | 免费 |
| **开源** | ✅ MIT | ❌ | ✅ Fair-code | ✅ MIT |
| **运行依赖** | ✅ 纯 Python3 标准库·零依赖 | 闭源 | 原生 | 原生 |
| **安装方式** | ✅ `pip`/`pipx` 一行 | DMG | DMG / brew | brew |
| **界面** | 本地 Web UI + CLI | 原生应用 | 原生应用 | 命令行 |
| **CLI / 脚本友好（`--json`）** | ✅ | ❌ | ❌ | ✅ |
| **删除默认进废纸篓** | ✅ | ✅ | ✅ | ➖ |
| **风险分级 + 危险项默认隐藏** | ✅ | ➖ | ➖ | ❌ |
| **删前 dry-run 逐项确认** | ✅ | ➖ | ✅ | ➖ |
...
| **误判排除清单（持久）** | ✅ | ➖ | ✅ | ❌ |
| **应用缓存清理** | ✅ | ✅ | ➖ | ✅ |
| **卸载残留检测（带理由可复核）** | ✅ bundle-id 精确 | ✅ | ✅ 专注 | ✅ |
| **大文件 / 重复文件** | ✅ / ✅ | ✅ / ✅ | ❌ | ❌ |
| **开发垃圾（Xcode/iOS/Android/鸿蒙）** | ✅ 含鸿蒙 | ➖ 仅 Xcode | ❌ | ➖ |
| **登录项 / 孤儿启动项** | ✅ | ✅ | ❌ | ➖ |

> ✅ 有 · ➖ 部分/有限 · ❌ 无。MacBroom 面向想要一个开源、零依赖、`pip` 一行就能装、默认安全、还能接进脚本/CI 的清理工具的人。

## 💡 为什么选 MacBroom？—— 从竞品的真实事故里长出来的

我们收集了**大量用户反馈的竞品痛点问题**，专门设计 MacBroom 来**避开它们已经发生过、被用户报告过的最痛事故**：
...
```
macbroom/
  cli.py             入口（参数解析、启动服务 / scan / doctor 子命令；console 命令 macbroom）
  __main__.py        支持 python -m macbroom
  doctor.py          环境预检（doctor 子命令：Python/macOS/完全磁盘访问/日志/端口）
  core/
