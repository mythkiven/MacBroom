...
- **操作审计日志 + 应用内查看器**：每次扫描与删除都写入 `~/Library/Logs/MacBroom/macbroom.log`（可用 `MACBROOM_LOG_DIR` 覆盖目录），并可在 App 内直接查看最近活动（设置 → 查看活动日志）。
...
这张表只对比 MacBroom 真正发力的维度：安全、开源、可脚本化的日常清理。
